"""EDC peak distance vs V_G sweep at Gamma.

Computes the EDC intensity profile at Gamma for a series of V_G values
(1–30 meV, 10 points), fits 4 Lorentzians, and plots the distance
between the TVB main peak and the side-band peak as a function of V_G.
...
def _base_filename():
    return (
        f"EDC_vs_V_"
        f"w1p{INTERLAYER['w1p']}_"
        f"w1d{INTERLAYER['w1d']}_"
        f"w2p{INTERLAYER['w2p']}_"
        f"w2d{INTERLAYER['w2d']}_"
        f"phiG{PHI_G_DEG:.0f}_"
        f"Vg{VG_MIN_MEV}-{VG_MAX_MEV}_"
        f"n{N_VG}"
    )


def compute_edc_distance(vg_ev, moire_ham, geometry):
    """Compute the distance (in meV) between TVB main and side-band peaks at Gamma."""
...
def main():
    ...
    for i, vg_meV in enumerate(vg_vals_meV):
        ...
        distances[i] = compute_edc_distance(vg_vals_ev[i], moire_ham, geometry)
    ...
    print("Computed distances (meV):")
    for vg, d in zip(vg_vals_meV, distances):
        print(f"  V_G = {vg:5.1f} meV  ->  {d:.3f} meV")
    ...
    print(f"\nSaved data: {OUTPUT_DIR / f'{base}.npz'}")

    fig, ax = plt.subplots(figsize=(8, 5), constrained_layout=True)

    ax.plot(vg_vals_meV, distances, "ko-", lw=1.5, markersize=7, label="Computed")
    ax.axhline(arpes_distance, color="red", lw=1.5, ls="--",
               label=f"ARPES = {arpes_distance:.1f} meV")

    ax.set_xlabel(r"$V_G$ (meV)", fontsize=12)
    ax.set_ylabel(r"$\Delta E$ (meV)", fontsize=12)
    ax.set_title(
        f"TVB – side band distance at \u0393\n"
        f"(w1p={INTERLAYER['w1p']}, w1d={INTERLAYER['w1d']}, "
